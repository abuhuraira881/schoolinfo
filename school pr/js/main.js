...
document.addEventListener('DOMContentLoaded', () => {
    ...
    buyBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // Fetch attributes from clicked button
            const title = btn.getAttribute('data-title');
            const price = btn.getAttribute('data-price');
            const duration = btn.getAttribute('data-duration');
            const desc = btn.getAttribute('data-desc');
            const modulesRaw = btn.getAttribute('data-modules');

            // Set Content inside Modal
            modalTitle.textContent = title;
            modalPrice.textContent = `Fee: ${price}`;
            modalDuration.textContent = `Duration: ${duration}`;
            modalDesc.textContent = desc;

            // Render Modules as List Items
            modalModules.innerHTML = '';
            if (modulesRaw) {
                const modulesArray = modulesRaw.split(',');
                modulesArray.forEach(mod => {
                    const li = document.createElement('li');
                    li.textContent = mod.trim();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    buyBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // Extract course data attributes
            const title = btn.getAttribute('data-title');
            const price = btn.getAttribute('data-price');
            const duration = btn.getAttribute('data-duration');
            const desc = btn.getAttribute('data-desc');
